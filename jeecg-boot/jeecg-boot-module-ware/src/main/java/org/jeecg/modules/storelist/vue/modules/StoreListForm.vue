        ...
        <a-row>
          <a-col :span="12">
            <a-form-item label="器材编号" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="['wareId']" placeholder="请输入器材编号" disabled ></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="12">
          ...
          <a-col :span="12">
            <a-form-item label="包装类型" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <j-dict-select-tag type="list" v-decorator="['packType']" :trigger-change="true" dictCode="ware_type" placeholder="请选择包装类型" disabled/>
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="器材数量" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input-number v-decorator="['wareNum', validatorRules.wareNum]" placeholder="请输入器材数量" style="width: 100%" disabled/>
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="库房绑定" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <j-dict-select-tag type="list" v-decorator="['depotBind']" :trigger-change="true" dictCode="ware_depot,depot_name,id" placeholder="请选择库房绑定" disabled/>
            </a-form-item>
          </a-col>
          <a-col :span="12">
          ...
          <a-col :span="12">
            <a-form-item label="入库时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <j-date placeholder="请选择入库时间" v-decorator="['enterTime']" :trigger-change="true" :show-time="true" date-format="YYYY-MM-DD HH:mm:ss" style="width: 100%" disabled/>
            </a-form-item>
          </a-col>
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'wareId','kitName','packType','wareNum','depotBind','goodsBind','qrBind','rfidBind','enterTime'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'wareId','kitName','packType','wareNum','depotBind','goodsBind','qrBind','rfidBind','enterTime'))
      },
    }
  }
</script>