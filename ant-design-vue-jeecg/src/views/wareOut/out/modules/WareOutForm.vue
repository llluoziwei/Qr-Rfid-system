        ...
        <a-row>
          <a-col :span="24">
            <a-form-item label="二维码绑定" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="['qrBind']" placeholder="请输入二维码绑定" @change="qrChange($event)"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="24">
          ...
          <a-col :span="24">
            <a-form-item label="出库时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <j-date
                placeholder="请选择出库时间"
                v-decorator="['outTime', validatorRules.outTime]"
                :trigger-change="true"
                :show-time="true"
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        add: '/wareout/wareOut/add',
        edit: '/wareout/wareOut/edit',
        queryById: '/wareout/wareOut/queryById',
      },
    }
  },
  computed: {
  ...
  methods: {
    // 获取二维码编号
    qrChange(e) {
      let enterList = '/wareenter2/wareEnter2/list'
      getAction(enterList).then((res) => {
        res.result.records.map((item) => {
          // console.log(item)
          if (item.qrBind === e.target.value) {
            this.validatorRules.wareId.initialValue = item.wareId
            this.validatorRules.wareNum.initialValue = item.wareNum
            this.validatorRules.kitName.initialValue = item.kitName_dictText
            console.log('same')
          } else if (e.target.value == '') {
            this.validatorRules.wareId.initialValue = ''
            this.validatorRules.kitName.initialValue = ''
          }
        })
      })
    },
    ...
    edit(record) {
      ...
      this.visible = true
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.model, 'qrBind', 'wareId', 'wareNum', 'kitName', 'rfidBind', 'outTime'))
      })
    },
    //渲染流程表单数据
    showFlowData() {
    ...
    popupCallback(row) {
      this.form.setFieldsValue(pick(row, 'qrBind', 'wareId', 'wareNum', 'kitName', 'rfidBind', 'outTime'))
    },
  },
}
</script>